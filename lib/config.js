...
function parseKey(fileContents, key) {
  ...
  if (atom.config.get("linter-cnew.gccDebug")) {
    console.log("LinterCNew: parsing config key "+key+" ("+fileContents[key]+")");
  }
  ...
  if (atom.config.get("linter-cnew.gccDebug")) {
    console.log("LinterCNew: type mismatch for "+key+" "+typeof(fileContents[key])+" vs "+CONFIG_KEYS[key]);
  }
}
...
function checkConf(confObj) {
  for (var key in CONFIG_KEYS) {
    if (confObj[key] == undefined) {
      if (atom.config.get("linter-cnew.gccDebug")) {
        console.log("LinterCNew: Missing key: "+key);
      }
      return false;
    }
  }
...
function addDefaultConf(confObj) {
  for (var key in CONFIG_KEYS) {
    if (!confObj[key]) {
      if (atom.config.get("linter-cnew.gccDebug")) {
        console.log("LinterCNew: Adding default value for: "+key);
      }
      if (atom.config.get("linter-cnew."+key)) {
        confObj[key] = atom.config.get("linter-cnew."+key);
      }
      else {
...
function readConfFile(fileName, confObj) {
  if (fs.existsSync(fileName)) {
    ...
    if (atom.config.get("linter-cnew.gccDebug")) {
      console.log("LinterCNew: Reading: "+fileName);
    }
