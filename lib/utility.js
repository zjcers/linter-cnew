...
module.exports = {
  ...
  getProjectBaseDir: function(openFile) {
    ...
    let projects = atom.project.getDirectories();
  ...
  buildCommand: function(activeEditor, file) {
    ...
    if (atom.config.get("linter-gcc.gccDebug")) {
      console.log("linter-gcc config: " + JSON.stringify(globalSettings[file]));
    }
  ...
  buildCommandFromSettings: function (fileName, editor, settings) {
    ...
    let flags = "";
    if (atom.config.get("linter-gcc.gccDebug")) {
      console.log("LinterGCC: gccDefaultCFlags: "+settings.gccDefaultCFlags);
      console.log("LinterGCC: gccDefaultCppFlags: "+settings.gccDefaultCppFlags);
    }
