...
module.exports = {
  ...
  getProjectBaseDir: function() {
    ...
    for (var i=0; i<projects.length; i++) {
      if (projects[i].contains(openFile)) {
        return projects[i].getPath();
      }
    }
  },
  /*@pre: a TextEditor must be open, otherwise getActiveTextEditor()
      will return undefined, which will throw a TypeError
    @returns: A Atom Directory object of the current project's
      base directory */
  getCwd: function() {
    return this.getProjectBaseDir();
  },
  getFileDir: function() {
    var filedir;
  ...
  buildCommand: function(activeEditor, file) {
    ...
    if (settings.compileCommandsFile) {
      ...
      } /*BUG: if settings.compileCommandsFile is just whitespace,
        correctly. Will refactor.*/
    } else {
